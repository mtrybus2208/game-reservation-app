...
export function* watchSocialLogin() {
  while (true) {
    yield take(actionTypes.SOCIAL_AUTH_GOOGLE);
    yield fork(workSocialAuth, 'google');

    yield take(actionTypes.SOCIAL_AUTH_GITHUB);
    yield fork(workSocialAuth, 'github');
  }
}
...
function savePlayer(mappedUser) {
  ...
  return axios.post(playerSaveLink, playerRequestBody);
}
